...
(function(addon) {
    ...
    UI.component('htmleditor', {
        ...
        init: function() {
            ...
            var $this = this, tpl = UI.components.aspxeditor.template;
            ...
            this.aspxeditor = $(tpl);
            this.content    = this.aspxeditor.find('.uk-htmleditor-content');
            this.toolbar    = this.aspxeditor.find('.uk-htmleditor-toolbar');
            this.preview    = this.aspxeditor.find('.uk-htmleditor-preview').children().eq(0);
            this.code       = this.aspxeditor.find('.uk-htmleditor-code');

            this.element.before(this.aspxeditor).appendTo(this.code);
            this.editor = this.CodeMirror.fromTextArea(this.element[0], this.options.codemirror);
            this.editor.aspxeditor = this;
            ...
            var previewContainer = this.iframe ? this.preview.container:$this.preview.parent(),
                ...
                codeScroll       = this.code.find('.CodeMirror-scroll').on('scroll', UI.Utils.debounce(function() {
                    ...
                    if ($this.aspxeditor.attr('data-mode') == 'tab') return;
            ...
            this.aspxeditor.on('click', '.uk-htmleditor-button-code, .uk-htmleditor-button-preview', function(e) {

                e.preventDefault();

                if ($this.aspxeditor.attr('data-mode') == 'tab') {

                    $this.aspxeditor.find('.uk-htmleditor-button-code, .uk-htmleditor-button-preview').removeClass('uk-active').filter(this).addClass('uk-active');

                    $this.activetab = $(this).hasClass('uk-htmleditor-button-code') ? 'code' : 'preview';
                    $this.aspxeditor.attr('data-active-tab', $this.activetab);
                    $this.editor.refresh();
                }
            });

            // toolbar actions
            this.aspxeditor.on('click', 'a[data-htmleditor-button]', function() {
            ...
            // autocomplete
            if (this.options.autocomplete && this.CodeMirror.showHint && this.CodeMirror.hint && this.CodeMirror.hint.aspx) {
                ...
                this.editor.on('inputRead', UI.Utils.debounce(function() {
                    ...
                    if (mode == 'xml') { //html depends on xml
                        ...
                        if (token.string.charAt(0) == '<' || token.type == 'attribute') {
                            $this.CodeMirror.showHint($this.editor, $this.CodeMirror.hint.aspx, { completeSingle: false });
                        }
                    }
                }, 100));
            }
            ...
            this.element.attr('data-uk-check-display', 1).on('uk-check-display', function(e) {
                if(this.aspxeditor.is(":visible")) this.fit();
            }.bind(this));
        ...
        fit: function() {
            ...
            if (mode == 'split' && this.aspxeditor.width() < this.options.maxsplitsize) {
                mode = 'tab';
            }
            ...
            if (mode == 'tab') {
                ...
                this.aspxeditor.find('.uk-htmleditor-button-code, .uk-htmleditor-button-preview').removeClass('uk-active')
            ...
            this.aspxeditor.attr('data-mode', mode);
        },
    ...
    UI.components.aspxeditor.template = [
    ...
    UI.plugin('htmleditor', 'base', {
        ...
        init: function(editor) {
            ...
            editor.aspxeditor.on('click', 'a[data-htmleditor-button="fullscreen"]', function() {
                editor.aspxeditor.toggleClass('uk-htmleditor-fullscreen');

                var wrap = editor.editor.getWrapperElement();

                if (editor.aspxeditor.hasClass('uk-htmleditor-fullscreen')) {
        ...
        init: function(editor) {
            ...
            editor.on('cursorMode', function(e, param) {
                if (editor.editor.options.mode == 'gfm') {
                    ...
                    if (!editor.editor.getTokenAt(pos).state.base.aspxState) {
                        param.mode = 'markdown';
                    }
                }
            });
            ...
            $.extend(editor, {
                ...
                disableMarkdown: function() {
                    ...
                    this.aspxeditor.find('.uk-htmleditor-button-code a').html(this.options.lblCodeview);
                    this.render();
                }
            ...
            function enableMarkdown() {
                ...
                editor.aspxeditor.find('.uk-htmleditor-button-code a').html(editor.options.lblMarkedview);
            }
    ...
    $(function() {
        $('textarea[data-uk-htmleditor]').each(function() {
            ...
            if (!editor.data('htmleditor')) {
                obj = UI.aspxeditor(editor, UI.Utils.options(editor.attr('data-uk-htmleditor')));
            }
        });
    });

    return UI.aspxeditor;
});